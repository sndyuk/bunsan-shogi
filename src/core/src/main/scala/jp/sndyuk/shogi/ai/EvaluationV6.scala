...
object EvaluationV6 {
  ...
  private val GOLD_PST_SENTE: Array[Array[Int]] = Array(
    ...
    Array(0, 1, 2, 2, 2, 2, 2, 1, 0),
    Array(0, 1, 2, 3, 3, 3, 2, 1, 0),
    Array(0, 2, 3, 4, 4, 4, 3, 2, 0),
    Array(0, 2, 3, 4, 5, 4, 3, 2, 0),
    Array(0, 2, 3, 4, 4, 4, 3, 2, 0),
    Array(0, 1, 2, 3, 3, 3, 2, 1, 0),
    ...
    for (currentY <- 0 to 8; currentX <- 0 to 8) {
      ...
      if (piece != Piece.❏ && Piece.▲△(piece, playerTurn)) {
        ...
        generalizedPiece match {
          ...
          case Piece.◯.KI => // Gold General
            val y = if (playerTurn == PlayerA) currentY else 8 - currentY
            val x = if (playerTurn == PlayerA) currentX else 8 - currentX
            pstScore += GOLD_PST_SENTE(y)(x)
          case Piece.◯.GI => // Silver General
            val y = if (playerTurn == PlayerA) currentY else 8 - currentY
            val x = if (playerTurn == PlayerA) currentX else 8 - currentX
            pstScore += SILVER_PST_SENTE(y)(x)
